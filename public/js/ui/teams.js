            ...
            try {
                ...
                if (teams.length === 0) {
                    ...
                    for (const team of teams) {
                        ...
                        html += '<div class="bg-valCard border ' + st.b + ' rounded-2xl p-5 hover:shadow-lg transition">';
                        html += '<div class="flex items-center justify-between mb-3">';
                        html += '<div><h4 class="text-white font-bold text-base">' + window.escHtml(team.name) + '</h4>';
                        html += '<p class="text-[10px] text-gray-500 mt-0.5">Bởi: ' + window.escHtml(captainName) + '</p></div>';
                        html += '<span class="text-[10px] font-mono ' + st.bg + ' border px-2 py-0.5 rounded-full font-bold whitespace-nowrap">' + sl + '</span>';
                        html += '</div>';
                        ...
                        // Main members
                        html += '<p class="text-[9px] text-gray-500 uppercase font-bold tracking-wider mb-1"><i class="fa-solid fa-shield-halved mr-1"></i>Chính Thức (' + mains.length + '/5)</p>';
                        for (const rid of mains) {
                            const p = rosterMap[rid] || null;
                            const name = p ? p.displayName : rid;
                            const isCap = rid === team.captainDiscordId;
                            html += '<div class="flex items-center gap-2 bg-valBg/60 p-2 rounded-lg text-xs cursor-pointer hover:bg-valBg hover:border-valCyan/30 transition border border-transparent" onclick="openProfile(\'' + rid + '\')">';
                            html += '<img src="' + getAvatarUrl(rid, p?.discordAvatar, 24) + '" class="w-5 h-5 rounded-full border border-gray-700" onerror="this.style.display=\'none\'">';
                            html += '<div class="min-w-0 flex-1"><span class="text-gray-300 hover:text-valCyan">' + window.escHtml(name) + '</span>';
                            html += '<span class="text-[8px] text-gray-600 block truncate">' + (p ? (p.rank||'') + (p.peakRank ? ' · Peak ' + p.peakRank : '') : '') + '</span></div>';
                            html += '<span class="text-[9px] text-gray-600 shrink-0">' + (p ? (p.elo || 0) + 'elo · ' + getPtsFromRank(p.peakRank||p.rank) + 'đ' : '') + '</span>';
                            if (isCap) html += '<span class="text-[9px] bg-yellow-500/20 text-yellow-400 px-1.5 rounded font-bold shrink-0 ml-1">ĐT</span>';
                            html += '</div>';
                        }
                        for (let i = mains.length; i < 5; i++) {
                            html += '<div class="flex items-center gap-2 bg-valBg/30 border border-dashed border-gray-800 p-2 rounded-lg text-xs text-gray-600"><i class="fa-solid fa-plus-circle text-gray-700"></i><span>Trống</span></div>';
                        }

                        // Sub members
                        html += '<p class="text-[9px] text-gray-500 uppercase font-bold tracking-wider mt-2 mb-1"><i class="fa-solid fa-chair mr-1"></i>Dự Bị (' + subMembers.length + '/2)</p>';
                        for (const rid of subMembers) {
                            const p = rosterMap[rid] || null;
                            const name = p ? p.displayName : rid;
                            const isCap = rid === team.captainDiscordId;
                            html += '<div class="flex items-center gap-2 bg-valBg/60 p-2 rounded-lg text-xs cursor-pointer hover:bg-valBg hover:border-valCyan/30 transition border border-gray-700/30 opacity-60" onclick="openProfile(\'' + rid + '\')">';
                            html += '<img src="' + getAvatarUrl(rid, p?.discordAvatar, 24) + '" class="w-5 h-5 rounded-full border border-gray-700" onerror="this.style.display=\'none\'">';
                            html += '<div class="min-w-0 flex-1"><span class="text-gray-300 hover:text-valCyan">' + window.escHtml(name) + '</span>';
                            html += '<span class="text-[8px] text-gray-600 block truncate">' + (p ? (p.rank||'') + (p.peakRank ? ' · Peak ' + p.peakRank : '') : '') + '</span></div>';
                            html += '<span class="text-[9px] text-gray-600 shrink-0">' + (p ? (p.elo || 0) + 'elo · ' + getPtsFromRank(p.peakRank||p.rank) + 'đ' : '') + '</span>';
                        ...
                        for (let i = subMembers.length; i < 2; i++) {
                            html += '<div class="flex items-center gap-2 bg-valBg/30 border border-dashed border-gray-700/50 p-2 rounded-lg text-xs text-gray-600"><i class="fa-solid fa-plus-circle text-gray-700"></i><span>Trống</span></div>';
                        }

                        html += '</div>';

                        html += '<div class="text-[11px] text-yellow-400 font-mono font-black mb-3 flex items-center gap-1 flex-wrap"><span class="bg-yellow-400/15 border border-yellow-400/25 px-2.5 py-0.5 rounded-lg">' + activePts + 'đ đánh chính</span><span class="text-gray-500 text-[9px] ml-1">(tổng ' + totalPts + 'đ)</span></div>';

                        const isCompleteOrApproved = team.status === 'approved' || team.status === 'complete';
                        html += '<div class="flex gap-2">';
                        if (isMember) {
                            ...
                            if (isCompleteOrApproved) {
                                html += '<span class="flex-1 text-center text-[11px] bg-emerald-500/10 text-emerald-400 border border-emerald-400/30 px-3 py-2 rounded-lg font-bold">✅ ' + (team.status === 'complete' ? 'Hoàn chỉnh' : 'Đã duyệt') + '</span>';
                            } else if (team.status === 'pending') {
                                html += '<span class="flex-1 text-center text-[11px] bg-yellow-500/10 text-yellow-400 border border-yellow-400/20 px-3 py-2 rounded-lg font-bold"><i class="fa-solid fa-clock mr-1"></i>Chờ Admin duyệt</span>';
                            } else if (team.status === 'rejected') {
                                html += '<span class="flex-1 text-center text-[11px] bg-red-500/10 text-red-400 border border-red-400/20 px-3 py-2 rounded-lg font-bold"><i class="fa-solid fa-ban mr-1"></i>Đã từ chối</span>';
                            } else if (team.status === 'recruiting') {
                                html += '<span class="flex-1 text-center text-[11px] bg-blue-500/10 text-blue-400 border border-blue-400/20 px-3 py-2 rounded-lg font-bold">📢 Tuyển thành viên</span>';
                            }
                        }
                        html += '</div></div>';
                    }
